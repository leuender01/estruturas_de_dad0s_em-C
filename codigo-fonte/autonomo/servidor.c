...
void calcular_chave_websocket(const char *key, char *output);//calcula o hash para realizar  handshake
...
int main(void)
{
    ...
    inet_aton("127.0.0.1", &(myserve.sin_addr));
    ...
    while(1)
    {
        ...
        {
            resposta = strdup("HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n");
        }
...
char *html(const char *nome_arquivo, const char *type)
{
    ...
    if(arquivo == NULL  )
    {
        perror("Erro abrir aqruivo");
        char *erro404 = strdup("HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n");
        return erro404;
